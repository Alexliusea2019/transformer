...
def run_validation(model, val_loader, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, num_examples=2):
    ...
    for batch in val_loader:
        ...
        if num_examples > 0:
            ...
            num_examples -= 1
        # once we've printed & collected N examples, stop decoding the rest
        if num_examples <= 0:
            break

    bleu_metric = torchmetrics.text.bleu.BLEUScore(n_gram=4)
    bleu_score = bleu_metric(predictions, [[ref] for ref in actuals])

    wandb.log({
        'validation/BLEU': bleu_score.item(),
